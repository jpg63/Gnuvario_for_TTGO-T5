...
//    SerialPort.println("beeper update");
  ...
  else
  {
    ...
    /*    (after parsing)      */
    /***************************/
    if (varioHardwareManager.variometerState < VARIOMETER_STATE_FLIGHT_STARTED)
    {

      /* if initial state check if date is recorded  */
      if (varioHardwareManager.variometerState == VARIOMETER_STATE_INITIAL)
      {
        if (nmeaParser.haveDate())
        {
          ...
          varioHardwareManager.variometerState = VARIOMETER_STATE_DATE_RECORDED;
        }
      }

      /* check if we need to calibrate the altimeter */
      else if (varioHardwareManager.variometerState == VARIOMETER_STATE_DATE_RECORDED)
      {
        ...
        if (nmeaParser.haveNewAltiValue() && nmeaParser.precision < VARIOMETER_GPS_ALTI_CALIBRATION_PRECISION_THRESHOLD)
        {
          ...
          if (varioData.compteurGpsFix > NB_ACQUISITION_FIX_GPS)
          {
            ...
            varioHardwareManager.variometerState = VARIOMETER_STATE_CALIBRATED;
  ...
  if ((varioHardwareManager.variometerState >= VARIOMETER_STATE_DATE_RECORDED) && (nmeaParser.haveNewSpeedValue()))
  {
...
//   screen.updateData(DISPLAY_OBJECT_TEMPRATURE, temprature);
  ...
  if (varioData.displayLowUpdateState)
  {
    ...
    SerialPort.println(varioHardwareManager.variometerState);
#endif //PROG_DEBUG
  }
...
#ifdef AGL_MANAGER_H
    currentHeight = varioData.aglManager.getHeight();
#ifdef PROG_DEBUG
...
void enableflightStartComponents(void)
{
  ...
  varioHardwareManager.variometerState = VARIOMETER_STATE_FLIGHT_STARTED;
