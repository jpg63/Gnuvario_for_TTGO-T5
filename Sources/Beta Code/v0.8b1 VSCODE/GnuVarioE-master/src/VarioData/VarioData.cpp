...
/*			if ((millis() > GnuSettings.FLIGHT_START_MIN_TIMESTAMP) &&
      ...
      if (!GnuSettings.VARIOMETER_RECORD_WHEN_FLIGHT_START)
      {
          //Demarre l'enregistrement dès que le GPS Fix
          enableFlightStart = true;
      }
      else if (millis() > GnuSettings.FLIGHT_START_MIN_TIMESTAMP)
      {
          //Attend au moins FLIGHT_START_MIN_TIMESTAMP milliseconde depuis le démarrage 

        if (((kalmanvert.getVelocity() < GnuSettings.FLIGHT_START_VARIO_LOW_THRESHOLD) || 
             (kalmanvert.getVelocity() > GnuSettings.FLIGHT_START_VARIO_HIGH_THRESHOLD)) && 
             (nmeaParser.getSpeed() > GnuSettings.FLIGHT_START_MIN_SPEED))
          enableFlightStart = true;      
      }
      
      if (enableFlightStart)
			{
